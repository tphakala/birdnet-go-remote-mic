...
spec), `licenses:verify` (THIRD_PARTY_LICENSES.md and web/static/licenses.json
match what the binary links), gofmt, and `go test -race`.
...
with its license file beside it). The same run writes `web/static/licenses.json`
(the same data plus remote-mic's own LICENSE and NOTICE), which the web UI's
About page loads. Regenerate and commit both with any change that alters the
linked modules; the Licenses workflow does this itself on Dependabot pull
requests. remote-mic itself is Apache-2.0 (`LICENSE`, `NOTICE`).
