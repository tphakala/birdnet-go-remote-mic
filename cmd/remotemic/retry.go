...
const (
	...
	// dies right away (an EIO after open, an encoder the stage rejects at start)
	// would otherwise publish a clear and a fresh onset, and rebuild the
	// advertisement, on every attempt. Its RTSP paths serve for the whole window.
	// The guard cannot see a fault in the encode itself: a stage encodes only
	// while a client plays (see pipeline.Stage), so such a fault would surface
	// at each PLAY rather than at open. No encode path is known to fail at
	// runtime (the frame length and output buffer are fixed and periods are
	// whole frames); if one appears, settle only after real encode work.
