...
function defaultOpusBitrate(channels) {
...
export class DeviceSettingsForm {
    ...
    bitrateFollows = false;
    settingBitrate = false;
    modeHidden;
    bitrateField;
    quietAlertEl;
    rateDrop;
    ...
    build() {
        ...
        const name = this.field(grid, `set-${uid}-name`, "Device Name", initialName, "text", "DNS-SD instance name and log label. Must be unique.");
        this.nameEl = name.input;
        this.nameErr = name.error;
        const path = this.field(grid, `set-${uid}-path`, "RTSP Path", d.path, "text", "Unique endpoint path on the RTSP server, e.g. /stream.");
        ...
        const modeHint = this.hint(opusOffered
            ? "Opus is 48 kHz (mono or stereo); PCM L16 is raw and supports ultrasonic rates."
            : "PCM L16 is raw and supports ultrasonic rates. Opus needs 48 kHz, which this device does not support.", `set-${uid}-mode-hint`);
        ...
        this.bitrateHidden.addEventListener("change", () => {
            if (this.ready && !this.settingBitrate)
                this.bitrateFollows = false;
        });
        this.bitrateField.appendChild(bitrate.container);
        const bitrateHint = this.hint("Target bitrate for the Opus encoder. Defaults to 128 kbps per channel (128 kbps mono, 256 kbps stereo).", `set-${uid}-bitrate-hint`);
        ...
        this.modeHidden.addEventListener("change", () => {
            ...
            this.applyChannelMode(this.modeHidden.value);
            if (isOpus) {
    ...
    syncDefaultBitrate() {
        if (this.modeHidden.value !== "opus")
            return;
        if (!this.bitrateFollows)
            return;
        const want = String(defaultOpusBitrate(this.selectedChannels().length));
        if (this.bitrateHidden.value === want)
            return;
    ...
    validate() {
        ...
        ok = this.markControl(this.rateErr, rateOk, mode === "opus"
    ...
    groupTitle(grid, text) {
        grid.appendChild(elem("div", "form-group-title", text));
    }
    buildDropdown(ariaLabel, options, selected) {
        ...
        for (const opt of options) {
            ...
            if (opt.tag)
                item.dataset.tag = opt.tag;
            if (opt.tagClass)
                item.dataset.tagClass = opt.tagClass;
    ...
    field(grid, id, labelText, value, type, hint) {
    ...
    channelHint(mode) {
        if (mode === "opus")
            return "Opus streams one or two channels: pick one for mono or two for stereo.";
        if (this.opusSupported())
            return "Select which capture channels to stream. One channel is a mono stream; Opus takes one or two.";
    ...
    applyChannelMode(mode) {
        ...
        this.channelsGroup.setAttribute("aria-label", opus ? "Capture channels to stream (Opus: one or two channels)" : "Capture channels to stream");
    }
    error(id) {
