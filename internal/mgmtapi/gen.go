...
type Notification struct {
	...
	// UptimeMs Milliseconds since the notification center started (early in process startup), read from a monotonic clock that a wall-clock step does not move. The difference between two entries' uptimeMs is the true time between them, and the snapshot's uptimeMs gives the uptime at a known moment, so a client can place any entry relative to when it received the snapshot.
	//
	//
	// Examples: 93500
...
type NotificationSnapshot struct {
	...
	// UptimeMs The notification center's uptime in milliseconds at the same instant as serverTime. An entry happened (uptimeMs minus the entry's uptimeMs) milliseconds before the snapshot, whatever its wall-clock time says.
	//
	//
	// Examples: 120000
	UptimeMs int64 `json:"uptimeMs"`
...
type SystemInfo struct {
	...
	// CpuPercent Host CPU utilization across all cores, as a percentage, averaged over the window between the server's two latest CPU readings. The readings are shared by all clients; a request less than a second after the latest one reuses its figure. The window is at most 30 seconds long: when the latest reading is older than that, the server samples for a quarter second before responding. Absent only when the host's CPU counters cannot be read or give no basis for a ratio.
